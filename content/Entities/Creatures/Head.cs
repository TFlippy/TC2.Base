	...
	public static partial class Head
	{
		...
		[Source.Owned, Override] in Organic.Data organic, [Source.Owned] ref Organic.State organic_state)
		{
			...
			if (time >= head_state.t_next_sound)
			{
				...
				if (species_data.IsNotNull())
				{
					head_state.t_next_sound = random.NextFloatExtra(time + 1.50f, 4.00f);
		...
		{
			no_rotate.multiplier *= Maths.Round(organic_state.consciousness_shared * organic_state.efficiency * Maths.Lerp(0.20f, 1.00f, organic.motorics * organic.motorics)) * organic.coordination * organic.motorics * (1.00f - organic_state.stun_norm);
