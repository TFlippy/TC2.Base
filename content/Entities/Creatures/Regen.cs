...
namespace TC2.Base.Components
{
	public static partial class Regen
	{
		...
		[Source.Owned, Original] ref Regen.Data regen, [Source.Owned] in Health.Data health)
		{
			...
			regen.t_next_regen = Maths.Max(regen.t_next_regen, info.WorldTime + (random.NextFloatExtra(regen.interval + 10.00f, 10.00f) * Constants.Health.regen_interval_modifier));
			//App.WriteValue(regen.t_next_regen);

		}

		[ISystem.PostUpdate.D(ISystem.Mode.Single, ISystem.Scope.Region, interval: 0.22f), HasTag("dead", false, Source.Modifier.Owned)]
			...
			if (info.WorldTime >= regen.t_next_regen)
			{
				if (health.integrity < 1.00f || health.durability < 1.00f)
				{
					entity.Heal(ent_healer: entity,
						...
						amount: regen_override.amount * (random.NextFloatRange(0.80f, 1.00f) * Constants.Health.regen_amount_modifier),
					...
					regen.t_next_regen = info.WorldTime + (regen_override.interval * Constants.Health.regen_interval_modifier);
				}
				else
				{
					regen.t_next_regen = info.WorldTime + (10.00f * Constants.Health.regen_interval_modifier);
				}
			}
		}

		[ISystem.PostUpdate.D(ISystem.Mode.Single, ISystem.Scope.Region, interval: 0.22f), HasTag("wrecked", false, Source.Modifier.Owned), HasComponent<Organic.Data>(Source.Modifier.Owned, false)]
