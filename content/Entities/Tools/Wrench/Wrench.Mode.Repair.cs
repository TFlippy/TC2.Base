	...
	public static partial class Wrench
	{
		public static partial class Mode
		{
			public static partial class Repair
			{
				...
				[IComponent.Data(Net.SendType.Reliable, IComponent.Scope.Region, name: "Wrench (Repair)", order: 9)]
...
#if SERVER
				[ISystem.Update.A(ISystem.Mode.Single, ISystem.Scope.Region, interval: 0.60f), HasTag("dead", false, Source.Modifier.Parent), HasRelation(Source.Modifier.Owned, Relation.Type.Stored, false)]
				...
				public struct TargetInfo: ITargetInfo
				{
					...
					public readonly IComponent.Handle ComponentID => ECS.GetID<Repairable.Data>();
					...
					public TargetInfo(Entity entity)
					{
						...
						if (this.alive)
						{
							...
							this.valid &= this.entity.GetComponent<Repairable.Data>().TryGetValueFromRef(out this.repairable);
							this.valid &= this.entity.GetComponent<Health.Data>().TryGetValueFromRef(out this.health);
